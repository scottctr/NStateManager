...
namespace NStateManager.Tests.Export
{
    public class StateMachineTests
    {
        ...
        public void GetSummary_includes_all_transitions()
        {
            ...
            Assert.Contains(summary.Transitions, t => t.FromState.State.CompareTo(SaleState.Open) == 0 && t.ToState.State.CompareTo(SaleState.Complete) == 0 && t.HasCondition);
            Assert.Contains(summary.Transitions, t => t.FromState.State.CompareTo(SaleState.Open) == 0 && t.ToState.State.CompareTo(SaleState.ChangeDue) == 0 && t.HasCondition);
            Assert.Contains(summary.Transitions, t => t.FromState.State.CompareTo(SaleState.ChangeDue) == 0 && t.ToState.State.CompareTo(SaleState.Complete) == 0 && !t.HasCondition);
        }
        ...
        private StateMachine<Sale, SaleState, SaleEvent> getTestStateMachine()
        {
            ...
            stateMachine.ConfigureState(SaleState.Open)
               .AddTransition(SaleEvent.Pay, SaleState.Complete, sale1 => Math.Abs(sale1.Balance) < 0.00001)
               .AddTransition(SaleEvent.Pay, SaleState.ChangeDue, sale2 => sale2.Balance < 0);
